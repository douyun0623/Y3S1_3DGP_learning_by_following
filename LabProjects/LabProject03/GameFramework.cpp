...
void CGameFramework::CreateSwapChain()
{
	...
	// 버포 용도 : 렌더 타겟 출력 용도로 사용한다.
	dxgiSwapChainDesc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;

	// 스왑 체인에 사용할 버퍼 수 ( 보통 2 ) 
	dxgiSwapChainDesc.BufferCount = m_nSwapChainBuffers;

	// 크기 조절 방식 ( 크기 고정 )
	dxgiSwapChainDesc.Scaling = DXGI_SCALING_NONE;

	// 교체 방식 : DXGI_SWAP_EFFECT_FLIP_DISCARD (최신 방식)
	dxgiSwapChainDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD;
	// 플립(flip) 방식으로 버퍼를 교체하며, 이전 백버퍼의 내용은 버림(discard)

	// 알파 모드/ 추가 플래그 설정 없음
	dxgiSwapChainDesc.AlphaMode = DXGI_ALPHA_MODE_UNSPECIFIED;
	dxgiSwapChainDesc.Flags = 0;

	// DXGI_SWAP_CHAIN_FULLSCREEN_DESC 설정
	DXGI_SWAP_CHAIN_FULLSCREEN_DESC dxgiSwapChainFullScreenDesc;
	::ZeroMemory(&dxgiSwapChainFullScreenDesc, sizeof(DXGI_SWAP_CHAIN_FULLSCREEN_DESC));
	dxgiSwapChainFullScreenDesc.RefreshRate.Numerator = 60;	// 리프레시율은 60Hz
	dxgiSwapChainFullScreenDesc.RefreshRate.Denominator = 1; 
	dxgiSwapChainFullScreenDesc.ScanlineOrdering = DXGI_MODE_SCANLINE_ORDER_UNSPECIFIED;
	dxgiSwapChainFullScreenDesc.Scaling = DXGI_MODE_SCALING_UNSPECIFIED;
	dxgiSwapChainFullScreenDesc.Windowed = TRUE;	// 전체 화면 모드에 대한 설정이지만, 여기서는 Windowed = TRUE로 창 모드 사용
	...
	// 지금 현재 gpu가 렌더링해야하는 버퍼 인덱스를 저장한다.
}
